...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy( w => w).map( x => (x._1, x._2.length) ).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map(e => (e._1, e._2 - y.find(_._1 == e._1).getOrElse(('_', 0))._2)).filter(_._2 != 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          occurCombination <- combinations(occurrences)
          word <- dictionaryByOccurrences(occurCombination)
          rest <- sentenceAnagrams(subtract(occurrences, occurCombination))
        } yield word :: rest
      }
    }
